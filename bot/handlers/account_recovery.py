...
async def handle_wallet_address(
    ...
    **data: Any,
    ...
    has_deposits = 'Да' if recovery_info.get('has_deposits') else 'Нет'
    has_balance = 'Да' if recovery_info.get('has_balance') else 'Нет'

    text = (
        f"✅ Кошель найден в системе.\n\n"
        f"**Информация об аккаунте:**\n"
        f"• Есть депозиты: {has_deposits}\n"
        f"• Есть баланс: {has_balance}\n\n"
        f"📝 **Шаг 2:** Подпишите следующее сообщение "
        f"своим приватным ключом:\n\n"
        f"```\n"
        ...
        f"2. Используйте MetaMask, Trust Wallet, "
        f"SafePal или другой кошелек\n"
        f"3. Найдите функцию 'Sign Message' "
        f"или 'Подписать сообщение'\n"
...
async def handle_signature(
    ...
    **data: Any,
    ...
    if not new_telegram_id:
        error_msg = "❌ Ошибка: не удалось определить ваш Telegram ID"
...
async def handle_additional_info(
    ...
    **data: Any,
    ...
    if not success or not user:
        error_message = third_value
        err_msg = error_message or 'Неизвестная ошибка'
        await message.answer(
            f"❌ Ошибка восстановления: {err_msg}\n\n"
