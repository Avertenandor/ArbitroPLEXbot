    ...
    **data: Any,
    ...
    if last_broadcast:
        ...
        if remaining_cooldown > 0:
            ...
            cooldown_msg = (
                f"⏳ Подождите {remaining_minutes} мин. "
                "перед следующей рассылкой"
            )
            await message.answer(
                cooldown_msg,
...
⚠️ Сообщение получат все зарегистрированные
пользователи.
⚙️ Рассылка использует ограничение
**15 сообщений/сек**.

**Поддерживается:**
• **Текст** — Просто отправьте текстовое сообщение
  (поддерживается Markdown)
• **Фото** — Прикрепите фото и добавьте текст
  в caption
• **Голосовые** — Отправьте голосовое сообщение
  (caption опционален)
• **Аудио** — Отправьте аудиофайл
  (caption опционален)

**Теперь можно добавить кнопку-ссылку!**
После отправки сообщения бот предложит
добавить кнопку с ссылкой на сайт или канал.
    """.strip()
    ...
    **data: Any,
    ...
    if message.text:
        ...
        error_msg = (
            ...
            "Используйте текст, фото, видео, голосовое, "
...
async def handle_button_link(
    ...
    **data: Any,
    ...
    if "|" not in text:
        ...
        await message.reply(format_error, parse_mode="Markdown")
        return
    ...
    if not url.startswith("http") and not url.startswith("t.me"):
        ...
        await message.reply(url_error, parse_mode="Markdown")
        return
