...
async def show_instructions(
    ...
    **data: Any,
    ...
    # R1-6: Проверка FSM состояния - если пользователь
    # в процессе регистрации, сбросить состояние и
    ...
    if current_state and current_state in [
        ...
        # R1-6: Пользователь в процессе регистрации,
        # нажал "📖 Инструкции"
        # Сбрасываем FSM состояние и показываем
        # главное меню
        await state.clear()
        if not user:
    ...
    if not user:
        ...
        instructions_text = (
            "📖 *О платформе ArbitroPLEXbot*\n\n"
            "ArbitroPLEXbot — это платформа для инвестиций "
            "в USDT на сети "
            "Binance Smart Chain (BEP-20).\n\n"
            "**Основные возможности:**\n"
            "• Инвестиции в USDT с автоматическими "
            "начислениями\n"
            ...
            "регистрацию.**\n"
            "Используйте кнопку '📝 Регистрация' или "
            "команду /start."
        )
    ...
    short_instructions_text = (
        "📖 *КАК СДЕЛАТЬ ДЕПОЗИТ*\n"
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "*1️⃣ Выберите уровень депозита*\n"
        "*2️⃣ Укажите сумму в пределах коридора*\n"
        "*3️⃣ Отправьте ТОЧНУЮ сумму в USDT*\n"
        "*4️⃣ Дождитесь подтверждения* (~5 мин)\n\n"
        "⚠️ *ВАЖНО:*\n"
        "• Только сеть *BSC (BEP-20)*\n"
        "• Только с *личного кошелька* (привязанного)\n"
        ...
        "_💎 PLEX: 10 монет за каждый $1 депозита в сутки_"
    )
...
async def show_detailed_instructions(
    ...
    **data: Any,
    ...
    if not user:
        await message.answer(
            "Для просмотра подробных инструкций "
    ...
    from app.config.business_constants import DEPOSIT_LEVELS

    detailed_instructions_text = (
        "📖 *Подробная инструкция по пополнению "
        "депозита*\n\n"
        "*1️⃣ Откройте ваш BSC кошелек* "
        "(Trust Wallet, MetaMask, SafePal или "
        "холодный кошелек)\n\n"
        "*2️⃣ Отправьте USDT (BEP-20)* на следующий адрес:\n"
        f"`{settings.system_wallet_address}`\n\n"
        "*3️⃣ Коридоры депозитов:*\n"
        f"   • Тестовый: ${DEPOSIT_LEVELS['test']['min']} - "
        f"${DEPOSIT_LEVELS['test']['max']}\n"
        f"   • Уровень 1: ${DEPOSIT_LEVELS['level_1']['min']} - "
        f"${DEPOSIT_LEVELS['level_1']['max']}\n"
        f"   • Уровень 2: ${DEPOSIT_LEVELS['level_2']['min']} - "
        f"${DEPOSIT_LEVELS['level_2']['max']}\n"
        f"   • Уровень 3: ${DEPOSIT_LEVELS['level_3']['min']} - "
        f"${DEPOSIT_LEVELS['level_3']['max']}\n"
        f"   • Уровень 4: ${DEPOSIT_LEVELS['level_4']['min']} - "
        f"${DEPOSIT_LEVELS['level_4']['max']}\n"
        f"   • Уровень 5: ${DEPOSIT_LEVELS['level_5']['min']} - "
        f"${DEPOSIT_LEVELS['level_5']['max']}\n\n"
        "*4️⃣ Дождитесь подтверждения* "
        "(обычно 1-3 минуты)\n\n"
        "*5️⃣ Депозит активируется автоматически* "
        "после 12 подтверждений блоков\n\n"
        f"📋 **УРОВНИ ДОСТУПА:**\n"
        f"```\n{LEVELS_TABLE}```\n"
        f"{RULES_SHORT_TEXT}\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n\n"
        ...
        "• Используйте личный кошелек "
        "(MetaMask, Trust Wallet, SafePal, Ledger)\n"
        "• 🚫 Не используйте вывод с бирж (Internal Transfer)\n"
        "• Убедитесь, что сумма находится в пределах "
        "коридора выбранного уровня\n"
        "• Сохраните hash транзакции для отслеживания\n\n"
        "*📋 Правила работы системы депозитов:*\n\n"
        "*Порядок покупки:*\n"
        "• Депозиты можно покупать только "
        "по возрастающей (тест→1→2→3→4→5)\n"
        "• Нельзя пропустить уровень "
        "(например, купить уровень 3 без уровня 2)\n"
        "• Тестовый уровень можно купить без партнеров\n"
        "• Для уровней 2+ требуется наличие активных "
        "партнеров уровня 1\n\n"
        "*Партнерская программа:*\n"
        "• Приглашайте друзей по вашей реферальной "
        "ссылке\n"
        "• Новый пользователь становится вашим "
        "партнером уровня L1\n"
        "• Вы получаете вознаграждения за активность "
        "партнеров\n"
        "• Партнеры влияют на возможность покупки "
        "более высоких уровней\n\n"
        "*ROI и ограничения:*\n"
        "• Для уровня 1 действует ROI cap 500% "
        "(максимум 5x от депозита)\n"
        "• Начисления происходят автоматически\n"
        "• Вывод средств доступен только после "
        "верификации\n\n"
        "*🌐 Подробная информация:*\n"
        "Больше информации о платформе, условиях и "
        "правилах можно найти на "
        "[официальном сайте](https://arbitrage-bot.com/).\n\n"
        f"*🔍 Проверить транзакцию:*\n"
        f"BSCScan: https://bscscan.com/address/"
        f"{settings.system_wallet_address}"
    )
