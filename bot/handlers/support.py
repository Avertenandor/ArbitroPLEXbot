...
async def handle_create_ticket(
    ...
    **data: Any,
    ...
    text = (
        ...
        "💡 **Совет:** "
        "Если вопрос касается финансов, укажите:\n"
        "• ID транзакции (Hash)\n"
        "• Сумму и дату\n\n"
    ...
    if not tickets:
        if user is None:
            text = (
                ...
                "Для создания обращения используйте кнопку "
                "'✉️ Создать обращение'."
            )
        else:
...
async def handle_faq(
    ...
    text = (
        ...
        "A: Выберите '💰 Депозит' → Выберите уровень → "
        "Отправьте USDT.\n"
        "⚠️ **Важно:** Только через сеть BSC (BEP-20) "
        "с личного кошелька!\n\n"
        "*Q: Сколько ждать подтверждения депозита?*\n"
        "A: Обычно 1-3 минуты после подтверждения "
        "в блокчейне.\n"
        "ℹ️ Требуется минимум 12 подтверждений "
        "в сети BSC.\n\n"
        "*Q: Как вывести средства?*\n"
        "A: Выберите '💸 Вывод' → Укажите сумму → "
        "Подтвердите фин. паролем.\n"
        ...
        "A: PLEX - внутренний токен платформы "
        "для комиссий.\n"
        "💡 Используется для оплаты транзакций, "
        "участия в арбитраже.\n"
        ...
        "A: Бот прекратит арбитражные операции → "
        "доход остановится.\n"
        "⚠️ **Важно:** Регулярно пополняйте PLEX "
        "для непрерывной работы!\n\n"
        "*Q: Откуда берется доход 30-70%?*\n"
        "A: От арбитража между DEX биржами (разница цен).\n"
        "💡 Бот автоматически находит выгодные сделки "
        "24/7.\n"
        "ℹ️ Доход зависит от волатильности рынка "
        "и уровня депозита.\n\n"
        ...
        "⚠️ Финпароль восстанавливается через "
        "'🔑 Восстановить финпароль'.\n\n"
        "*Q: Что делать если забыл финансовый пароль?*\n"
        "A: Используйте пункт "
        "'🔑 Восстановить финпароль'.\n"
        "ℹ️ Потребуется подтверждение "
        "через поддержку.\n\n"
        "*Q: Безопасно ли это?*\n"
        "A: Да! Мы используем:\n"
        ...
        "⚠️ **Никогда не сообщайте финпароль "
        "третьим лицам!**\n\n"
        "Для других вопросов создайте обращение "
        "в поддержку."
    )
