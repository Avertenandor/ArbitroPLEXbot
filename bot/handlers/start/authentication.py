...
ECOSYSTEM_INFO = (
    ...
    "Мы строим **крипто-фиатную экосистему** на базе "
    "монеты **PLEX** и высокодоходных торговых роботов.\n\n"
    ...
    **data: Any,
    ...
    if user and user.wallet_address:
        ...
        await callback.message.answer(
            ...
            "с которого был совершен перевод:\n"
            "Формат: `0x...`",
            parse_mode="Markdown"
        )
    ...
    try:
        ...
        if result["success"]:
            ...
            if db_user and db_session:
                ...
                await send(
                    "✅ **Оплата подтверждена!**\n\n"
                    "Для завершения регистрации создайте "
                    "финансовый пароль.\n\n"
            ...
            await send(
                "❌ **Оплата не найдена**\n\n"
                "Мы проверили последние транзакции, "
                "но не нашли поступления.\n"
                "• Убедитесь, что отправили 10 PLEX\n"
                "• Подождите 1-2 минуты, "
                "если транзакция еще в пути\n\n"
                "Попробуйте еще раз:",
    ...
    **data: Any,
    ...
    if verification.is_onchain_ok and plex_balance > 0:
        if not can_spend_plex(plex_balance, auth_price):
            ...
            await message.answer(
                f"⚠️ **Недостаточно свободных PLEX "
                f"для авторизации**\n\n"
                f"🔒 Несгораемый минимум: "
                f"**{MINIMUM_PLEX_BALANCE:,}** PLEX\n"
                f"💰 Ваш баланс: **{int(plex_balance):,}** PLEX\n"
                f"📊 Доступно для оплаты: "
                f"**{int(available):,}** PLEX\n"
                f"💳 Требуется для авторизации: "
                f"**{auth_price}** PLEX\n"
                f"📉 Не хватает: **{int(shortage)}** PLEX\n\n"
                f"Пополните баланс PLEX минимум на "
                f"**{int(shortage)}** токенов,\n"
                f"чтобы после оплаты на кошельке осталось "
                f"≥ {MINIMUM_PLEX_BALANCE:,} PLEX.",
                parse_mode="Markdown",
            )
