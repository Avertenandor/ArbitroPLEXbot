...
async def _get_last_scanned_block(redis_client, last_block_key):
    """Get the last scanned block from Redis."""
    if not redis_client:
        return None

    try:
        return await redis_client.get(last_block_key)
    except Exception as e:
        logger.warning(f"Failed to get last_scanned_block from Redis: {e}")
        return None


async def _update_last_scanned_block(redis_client, last_block_key, block_number):
    """Update the last scanned block in Redis."""
    if not redis_client:
        return

    try:
        await redis_client.set(last_block_key, str(block_number))
        logger.debug(f"Updated last_scanned_block to {block_number}")
    except Exception as e:
        logger.warning(f"Failed to update last_scanned_block in Redis: {e}")


def _determine_scan_range(last_scanned, current_block):
...
def _parse_transfer_log(log, w3):
    """Parse a transfer log event and extract transaction details."""
...
async def _process_transfer_logs(logs, service, w3):
    """Process all transfer log events."""
    for log in logs:
        try:
            transfer_data = _parse_transfer_log(log, w3)
            await service.process_incoming_transfer(**transfer_data)
        except Exception as e:
    ...
    async with lock.lock("incoming_transfer_monitoring", timeout=300):
        try:
            ...
            async with async_session_maker() as session:
                ...
                last_scanned = await _get_last_scanned_block(
                    redis_client, last_block_key
                )

                current_block = await blockchain.get_block_number()

                # Determine scan range
                from_block, to_block = _determine_scan_range(
                    last_scanned, current_block
                )

                logger.info(
                    f"Scanning blocks {from_block} to {to_block} for incoming USDT..."
                )
                ...
                padded_system_wallet = (
                    "0x" + settings.system_wallet_address[2:].lower().zfill(64)
                )
                ...
                await _process_transfer_logs(logs, service, w3)

                # Update last scanned block in Redis for next run
                await _update_last_scanned_block(
                    redis_client, last_block_key, to_block
                )
