    ...
    async def _process_deposit_internal(
        ...
        if not user:
            ...
            await self.notification_service.notify_admins(
                ...
                f"От: `{from_address}`\n"
                f"TX: `{tx_hash}`\n\n"
                f"Кошелек не привязан ни к одному "
                f"пользователю!\n"
        ...
        async with self.distributed_lock.lock(
            ...
            if active_deposits_count >= MAX_DEPOSITS_PER_USER:
                ...
                await self.notification_service.notify_user(
                    ...
                    f"Вы достигли лимита в "
                    f"{MAX_DEPOSITS_PER_USER} депозитов.\n"
                    f"Сумма: `{amount} USDT`\n"
                    f"TX: `{tx_hash}`\n\n"
                    "Дождитесь завершения одного из текущих "
                    "депозитов или обратитесь в поддержку."
                )
                await self.notification_service.notify_admins(
        ...
        try:
            ...
            try:
                ...
                await self.notification_service.notify_user(
                    ...
                    f"Необходимо оплатить: "
                    f"**{int(daily_plex_required):,} PLEX**\n\n"
                    f"💳 Кошелек для оплаты:\n"
                    f"`{settings.system_wallet_address}`\n\n"
                    f"После оплаты PLEX депозит начнет работать.\n"
                    f"Оплата требуется ежедневно "
                    f"(10 PLEX за каждый $1).\n"
                    f"Ваши индивидуальные сутки начнутся "
                    f"с момента первой оплаты."
                )
