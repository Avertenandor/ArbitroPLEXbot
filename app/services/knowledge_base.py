...
DEFAULT_KB: list[dict[str, Any]] = [
    ...
    {"id": 32, "category": "PLEX токен",
     ...
     "answer": (
         "Курс PLEX стабилизируется несколькими механизмами:\n\n"
         "1️⃣ ТРЕЙДЕРЫ — не работают с арбитроботом, торгуют только токеном. "
         "Их роботы при росте цены поэтапно продают небольшими кусочками.\n\n"
         "2️⃣ СИСТЕМА — получает PLEX напрямую от пользователей "
         "(10 за вход + 10 за каждый $1 депозита). "
         "Торговый робот системы следит за курсом и продаёт кусочками, "
         "если цена превышает оптимальный уровень.\n\n"
         "Почему не продаём сразу? Чтобы не ломать волатильность и дать "
         "холдерам время продать по хорошей цене, если нужно."
     ),
     "clarification": (
         "Это демонстрирует адекватность, гибкость и лояльность к холдерам. "
         "Система заботится о балансе роста и падения цены."
     ),
     "added_by": "system", "verified_by_boss": True},
    ...
    {"id": 43, "category": "Депозиты",
     ...
     "answer": (
         ...
         "Без оплаты PLEX депозит не участвует в арбитраже!"
     ),
    ...
    {"id": 56, "category": "Система",
     ...
     "answer": (
         "Рабочая сессия пользователя: 25 минут.\n"
         "При бездействии система автоматически сбросит вас.\n"
         "Для продолжения работы нужно снова нажать /start."
     ),
     "clarification": "Это мера безопасности для защиты вашего аккаунта.",
     "added_by": "system", "verified_by_boss": True},
